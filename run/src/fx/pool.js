...
export const FLAG = {
  HOMING: 1,     // к uTarget по smoothstep + дуга в сторону (магнит энергона)
...
void main(){
	...
	if ( fxFlag( fl, 1.0 ) > 0.5 ){
		// Магнит: smoothstep к цели + дуга В СТОРОНУ (знак из seed) — шлейф идёт сбоку от силуэта, а не сквозь спину.
		// 2.2 м на пике дуги: с игровой камеры (6.4 м сзади) полуширина Ризи ≈ 0.45 м — шлейф гарантированно снаружи.
		// Раньше здесь был easeInQuad (k·k): на любом отдельном кадре ВСЕ частицы кучковались у энергона
		// (k² почти не двигает первую половину жизни) и ленты не получалось. smoothstep раскладывает
		// одновременно живущие частицы равномерно по траектории — это и читается как шлейф.
		float hk = k * k * ( 3.0 - 2.0 * k );
