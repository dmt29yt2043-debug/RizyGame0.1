...
const rbox = (w, h, d, r = 0.06, s = 2) => new RoundedBoxGeometry(w, h, d, s, Math.min(r, w / 2 - 1e-3, h / 2 - 1e-3, d / 2 - 1e-3));
...
export function nearFence(){
  ...
  for (let i = 0; i < 6; i++) b.add(sph(0.05, 6, 4), ic, M(0, 0.7 - (i % 2) * 0.05, -0.25 - i * 0.5));
  return b.build();
}
...
export function nearSnowman(){
  ...
  b.add(sph(0.4, 14, 10), w, M(0, 0.36, 0), { ao: AO, keepUV: true });
  b.add(sph(0.29, 14, 10), w, M(0, 0.93, 0), { keepUV: true });
  b.add(sph(0.21, 14, 10), w, M(0, 1.35, 0), { keepUV: true });
...
export function obstSlideSeg(){
  ...
  for (let i = 0; i < 8; i++) bl.add(sph(0.055, 8, 6), 0xffffff, M(-1.12 + i * 0.32, 1.22, 0.07));
...
// WALL: сани / пирамида подарков / ледяная глыба (≥ 75% насыщенности, отделка чернилами ≤ 15%)
export function obstSleigh(){
  const b = new GeoBuilder(), or = 0xF07A2A, wh = 0xffffff, ink = 0x070D36, red = 0xFF3B5C;
...
export function obstGifts(){
  const b = new GeoBuilder(), or = 0xF07A2A, red = 0xFF3B5C, wh = 0xffffff;
  ...
  box(2.2, 1.3, 1.9, 0.65, 0, or);
  box(1.6, 0.95, 1.45, 1.78, 0.28, red);
  box(1.0, 0.62, 1.0, 2.56, -0.2, or);
  b.add(new THREE.TorusGeometry(0.2, 0.07, 8, 14), wh, M(-0.16, 3.0, 0, 0, 0.3, 0.7));
  b.add(new THREE.TorusGeometry(0.2, 0.07, 8, 14), wh, M(0.16, 3.0, 0, 0, 0.3, -0.7));
  return b.build();
}
export function obstIce(){
  const b = new GeoBuilder();
  b.add(rbox(2.3, 2.3, 1.9, 0.24, 3), 0x19B4F5, M(0, 1.2, 0));
  b.add(rbox(2.36, 0.26, 1.96, 0.12, 2), 0xffffff, M(0, 2.33, 0));
  for (const [x, y, rz] of [[-0.5, 1.4, 0.7], [0.35, 0.9, -0.5], [0.6, 1.7, 0.3]])
    b.add(rbox(0.7, 0.05, 0.02, 0.01), 0xDDF6FF, M(x, y, 0.96, 0, 0, rz));
  b.add(rbox(2.4, 0.12, 2.0, 0.04), 0x070D36, M(0, 0.06, 0));
  return b.build();
}
...
export function coinCore(){ const g = new THREE.IcosahedronGeometry(0.17, 0); g.scale(0.8, 1.15, 0.5); return g; }
